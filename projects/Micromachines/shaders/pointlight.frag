...
const int numLights = 2; // HAVE TO SET THIS MANUALLY
...
void main() {
	...
	for(int light = 0; light < numLights; light++){
		...
		if(Lights[light].isPointLight){
			...
			l = normalize(vec3(Lights[0].position) - DataIn.position);
			h = normalize(l + e);

			strength = 1.0f / (length(vec3(Lights[0].position) - DataIn.position)* Lights[light].linearAttenuation);
